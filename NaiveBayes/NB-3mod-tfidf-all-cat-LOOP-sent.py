...
files = ['silvics_edited_data_sents.csv', 'wikipedia_edited_data_sents.csv', 'merged_edited_data_sents.csv']
field = 'category'
gnb = GaussianNB()
cnb = ComplementNB()
mnb = MultinomialNB()
    ...
    for n in [10, 19]:
        ...
        for maxFeatures in [1000, 2000, 5000]:
            for test_set in [0.50, 0.25, 0.10]:
                ...
                print(f'{file}, {model_text}, {n}, {maxFeatures}, {test_set}, {f1_macro:.3f}, {accuracy:.3f}')
...
loop_data.to_csv('NB-3mod-tfidf-all-cat-LOOP-sent-data.csv')